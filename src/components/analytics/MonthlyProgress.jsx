...
function MonthlyProgress({ userId }) {
  ...
  const stats = useMemo(() => {
     ...
     * WEIGHTED COMPLETION
     *
     * Task counts remain actual counts.
     * Completion percentage uses W1-W4 weight.
     *
     * If weight is missing, default to W1.
     */
    const totalWeight =
      tasks.reduce(
        (sum, task) =>
    ...
    >
      ...
      >
        ...
        {stats.taskAllocation
          ...
          >
            No task-linked focus
            time this month.
          </p>
        ) : (
          <div>
            {stats.taskAllocation.map(
              task => {
                const percentage =
                  stats.totalFocus === 0
                    ? 0
